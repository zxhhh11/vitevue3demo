  ...
  <div class="customer-info-box">
      ...
      <el-tab-pane>
        <template #label>
          <span class="custom-tabs-label">
            <span>客户信息 </span>
            <el-button @click="showDetail('客户详细信息1')" key="primary" type="primary" link>[详细]</el-button>
    ...
    <el-tabs type="border-card">
      <el-tab-pane>
        ...
        <div>
          <BaseTable :tableData="cardLists" :total="listTotal" :currentPage="currentPage" :hasPagination="true"
            :handleClick="handleClick" :isMutiSelect="false" @handleCurrent="handleCurrent" :newColumns="columns">
          </BaseTable>
    ...
    <el-tabs type="border-card">
      <el-tab-pane>
        ...
        <div>
          <BaseTable :tableData="accountLists" :total="accountListTotal" :currentPage="accountCurrentPage"
            :hasPagination="true" :handleClick="handleClick" :isMutiSelect="false" @handleCurrent="handleCurrent"
            :newColumns="accountColumns"></BaseTable>
    ...
    <BaseModal :dialogVisiable="customerDetailVisiable" @hide-dialog="hideCardDialog" :title="title" cancelText="关闭"
      :hasConfirm="false" width="60%">
      <!-- 第一种list 渲染方式 list 和data 分别定义 支持插槽 支持content 加标签 但是可能不支持太过复杂的list  -->
      <CustomList :list="commonlist" :column="3" :datas="commondatas">
      </CustomList>
    </BaseModal>
    <BaseModal :dialogVisiable="cardVisiable" @hide-dialog="hideCardRemarkDialog" title="卡片备注信息" width="60%"
      :showFooter="false">
      ...
      <el-descriptions :column="1" border>
        <el-descriptions-item label="卡号" label-align="right" align="left">6259061121339689</el-descriptions-item>
        <el-descriptions-item label="备注信息" label-align="right" align="left">
          <el-input v-model="NOTEINFO" :rows="2" style="width:60%" type="textarea" placeholder="Please input" />
    ...
    <BaseModal :dialogVisiable="accoutDetailVisiable" @hide-dialog="hideaccountDialog" title="账户详细信息" cancelText="关闭"
      :hasConfirm="false" width="60%">
      <!--  另一种 list 遍历方式这种list和data在同一数组中 且这种方式可扩展性强 -->
      <el-descriptions :column="3" size="small" border>
        <template v-for="(item, key) in accountDetail2" :key="key">
          <el-descriptions-item :span="Number(item.span) ? Number(item.span) : 1">
            <template #label>
              {{ item.label }}
            </template>
...
import {
  customerInfoDatas, customerInfoList, cardList, accountList, customerDetailInfo,
  customerInfo, cardDetail, cardDetailInfo, accountDetail2
} from '@/utils/data'
